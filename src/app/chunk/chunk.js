...
const chunkModule = angular.module('resumeChunks', [])
.factory('resumeChunkService', ($rootScope, sideMenuEvents) => {
  let currentChunk = 'all';
  const chunks = {
    'skills': {
      'content': {
  ...
  const getChunk = (chunk) => {
    let chunkLookup = chunks[chunk];
    if(typeof chunkLookup === 'undefined' || chunkLookup === null) {
      chunkLookup.content = ['Chunk not found'];
    }
    return chunkLookup.content;
  };

  const getChunkIcon = (chunk) => {
    let chunkLookup = chunks[chunk];
    if(typeof chunkLookup === 'undefined' || chunkLookup === null) {
      chunkLookup = {};
      chunkLookup.icon = 'ion-help';
    }
    return chunkLookup.icon;
  };

  const getChunkNames = () => {
    return _.keys(chunks);
  };
...
.controller('chunkController', ($scope, resumeChunkService, sideMenuEvents) => {
  ...
  $scope.getChunk = () => {
    return resumeChunkService.getChunk($scope.name);
  };
  $scope.getIcon = () => {
